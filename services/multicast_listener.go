...
func ListenLocalSendMulticast(networkType string, multicastAddr string, multicastPort string, outboundInterface *net.Interface, sigCtx context.Context, chanMsg chan<- entities.UDPPacketData) {
	// for 循环保持连接
	for {
		exit, err := func() (bool, error) {
			...
			case "udp4":
				...
				if err := p4.JoinGroup(outboundInterface, &net.UDPAddr{IP: net.ParseIP(multicastAddr)}); err != nil {
					fmt.Printf("Error joining IPv4 multicast group: %v\n", err)
					return false, err
				}
			...
			case "udp6":
				...
				if err := p6.JoinGroup(outboundInterface, &net.UDPAddr{IP: net.ParseIP(multicastAddr)}); err != nil {
					fmt.Printf("Error joining IPv6 multicast group: %v\n", err)
					return false, err
				}
				packetConn = entities.PacketConn{
					IPv4Conn: nil,
		...
		if exit {
			...
			if err != nil {
				// 异常退出
				fmt.Printf("Multicast listener exited with error: %v\n", err)
			}
			break
		}

		fmt.Printf("Restarting multicast listener...Previous error: %v\n", err)
